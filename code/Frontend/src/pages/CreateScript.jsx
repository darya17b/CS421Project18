...
const CreateScript = () => {
  ...
  const [isCoreSummaryOpen, setIsCoreSummaryOpen] = useState(true);
  const [isReviewSymptomsOpen, setIsReviewSymptomsOpen] = useState(false);
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <form onSubmit={onSubmit} onKeyDown={onFormKeyDown} className="space-y-6 text-left">
          <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-6 space-y-3">
            <button
              type="button"
              onClick={() => setIsCoreSummaryOpen((prev) => !prev)}
              className="flex w-full items-center justify-between text-left"
            >
              <span className="text-base font-semibold text-gray-900">Script Summary</span>
              <span className="text-base font-semibold text-gray-700">{isCoreSummaryOpen ? "(-)" : "(+)"}</span>
            </button>
            {isCoreSummaryOpen ? (
          ...
          <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-6 space-y-3">
            <button
              type="button"
              onClick={() => setIsReviewSymptomsOpen((prev) => !prev)}
              className="flex w-full items-center justify-between text-left"
            >
              <span className="text-base font-semibold text-gray-900">Review of Symptoms</span>
              <span className="text-base font-semibold text-gray-700">{isReviewSymptomsOpen ? "(-)" : "(+)"}</span>
            </button>
            {isReviewSymptomsOpen ? (
            <>
